...
function buildOAuthWelcomeEmail(prenom: string, provider: string): string {
          ...
          <tr>
            <td class="email-body" style="padding:48px 48px 40px;">
              ...
              <p style="margin:0 0 32px;font-size:14px;line-height:1.8;color:#b0a898;letter-spacing:0.02em;">
                Nous sommes honor&eacute;s de vous accueillir au sein d&rsquo;<strong style="color:#f5f0e8;">Éloquence AI</strong> — la plateforme d&rsquo;entra&icirc;nement &agrave; l&rsquo;art de la parole. Votre compte ${provider === "google" ? "Google" : "Éloquence"} est actif et vous pouvez d&egrave;s maintenant acc&eacute;der &agrave; votre espace priv&eacute;.
              </p>
              ...
              <table role="presentation" cellspacing="0" cellpadding="0" border="0" class="email-cta" style="margin-bottom:36px;">
                <tr>
                  <td style="background:#c9a84c;">
                    <a href="https://www.xn--loquence-90a.fr/dashboard"
          ...
          <tr>
            <td align="center" style="padding:28px 48px;background:#0a0a0f;">
              ...
              <p style="margin:0 0 12px;">
                <a href="https://www.xn--loquence-90a.fr" style="font-size:11px;color:#c9a84c;text-decoration:none;letter-spacing:0.06em;">www.xn--loquence-90a.fr</a>
              </p>
...
export async function POST(req: NextRequest) {
  try {
    const { email, prenom, provider } = await req.json()
    ...
    const emailHtml = buildOAuthWelcomeEmail(prenom || "", provider || "google")
    ...
    try {
      emailRes = await fetch("https://api.brevo.com/v3/smtp/email", {
        ...
        body: JSON.stringify({
          sender: { name: "Éloquence AI", email: process.env.BREVO_SENDER_EMAIL || "eloquenceaii@gmail.com" },
    ...
    if (!emailRes.ok) {
      ...
      console.error("Brevo OAuth welcome email failed:", detail)
      return NextResponse.json({ ok: false, error: "Failed to send email" }, { status: 500 })
    }

    // Add/update contact in Brevo list
    ...
    console.error("OAuth welcome email error:", error)
